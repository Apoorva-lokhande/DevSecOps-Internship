...
This section aims to perform dynamic analysis using DAST tools on DVNA and solve the 8th point of the [Problem Statement](https://devsecops-report.netlify.app/problem-statements/).

## What is DAST
Dynamic application security testing (DAST) is the process of testing and evaluating a program while the software is running. Also referred to as dynamic code scanning, dynamic analysis improves the diagnosis and correction of bugs, memory issues, and crashes of an application during its execution.
...
I got a permission denied error, and resolved it by changing the permissions as below:
...
To run a full-scan script, run the following command:
...
The complete report generated by NodeJsScan, can be found [here](https://github.com/Apoorva-lokhande/DevSecOps-internship/blob/master/reports/zap-report.html).