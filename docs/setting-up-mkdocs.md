...
- Install the MKDocs package using:# Setting up MKDocs
...
## What is MKDocs
MkDocs is a static site generator that's geared towards building project documentation. Documentation source files are written in Markdown, and configured with a single YAML configuration file. MkDocs will build your docs and used them to commit and push them to GitHub.
...
- Later, check the version of the MKDocs installed to check everything worked okay.
  
        mkdocs --version

### Selecting a Theme

To customize a theme, I referred to the official [Documentation](https://github.com/mkdocs/mkdocs/wiki/MkDocs-Themes) from MkDocs themes. Here I preferred the `Material` theme, to use this particular theme it needs to be installed via pip. 

    pip install mkdocs-material

### Configuration

Create a new project to store all the documentation as given in the below commands:
...
`The index.md file is created in the docs folder which contains a single documentation page.
...
Here we have mentioned the site name as `DevSecOps-pipeline` which is the title of the site, in the `nav` includes all the contents which are explained throughout the documentation and the `theme` needs to be mentioned here.
...
Later, I checked the repository all the files were committed successfully.

### Building the MKDocs site

Building a site converts the documented files into an HTML and CSS format and will the files in a folder named `site` within the same directory which contains the `mkdocs.yml` file.

In the terminal I ran the following command from the parent directory: 
...
- Change the name of the site, as default it will be having a dummy name (in my case it is `devsecops-pipeline`).
